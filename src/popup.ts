...
document.addEventListener('DOMContentLoaded', () => {
  const highlightsList = document.getElementById('highlights-list') as HTMLDivElement;
  ...
  const closeSummaryBtn = document.getElementById('close-summary-btn') as HTMLButtonElement;
  ...
  chrome.windows.getCurrent((win) => {
    if (win.type === 'popup') {
      ...
      detachBtn.innerHTML = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"></rect><path d="M7 11V7a5 5 0 0 1 9.9-1"></path></svg>';
  ...
  function executeSummarize(highlights: Highlight[]) {
    chrome.storage.local.get(['groqApiKey'], (res) => {
      ...
      summaryContent.innerHTML = '<div class="loading"><div class="spinner"></div><span class="loading-text">Analyzing highlights...</span></div>';
  ...
  function loadHighlights() {
    chrome.storage.local.get({ highlights: [] }, (result) => {
      ...
      currentHighlights = rawHighlights.sort((a, b) => b.timestamp - a.timestamp);
  ...
  function renderHighlights(highlights: Highlight[]) {
    if (highlights.length === 0) {
      highlightsList.innerHTML = '<div class="empty-state">No highlights saved yet.<br>Select text on any webpage to begin!</div>';
  ...
  function deleteHighlight(id: string) {
    chrome.storage.local.get({ highlights: [] }, (result) => {
      const updated = (result.highlights as Highlight[]).filter(h => h.id !== id);
  ...
  async function generateSummary(apiKey: string, highlights: Highlight[]): Promise<string> {
    const combinedText = highlights.map((h, i) => `[${i+1}] From "${h.title}": "${h.text}"`).join('\n\n');
    const systemPrompt = highlights.length === 1 
      ? 'You are a helpful assistant. Provide a concise, insightful summary of this specific highlight under 100 words. Do not use generic introductions.'
